...
func NewServer(dir string, APIaddr string, requiredUserAgent string, requiredPassword string, cs modules.ConsensusSet, e modules.Explorer, fm modules.FeeManager, g modules.Gateway, h modules.Host, m modules.Miner, r modules.Renter, tp modules.TransactionPool, w modules.Wallet, mp modules.Pool, sm modules.StratumMiner, i modules.Index) (*Server, error) {
	return NewCustomServer(dir, APIaddr, requiredUserAgent, requiredPassword, cs, e, fm, g, h, m, r, tp, w, mp, sm, i, &modules.ProductionDependencies{})
}
...
func NewCustomServer(dir string, APIaddr string, requiredUserAgent string, requiredPassword string, cs modules.ConsensusSet, e modules.Explorer, fm modules.FeeManager, g modules.Gateway, h modules.Host, m modules.Miner, r modules.Renter, tp modules.TransactionPool, w modules.Wallet, mp modules.Pool, sm modules.StratumMiner, i modules.Index, apiDeps modules.Dependencies) (*Server, error) {
	...
	api := NewCustom(cfg, requiredUserAgent, requiredPassword, cs, e, fm, g, h, m, r, tp, w, mp, sm, i, apiDeps)
	srv := &Server{
		api: api,
		apiServer: &http.Server{
			Handler: api,
...
func assembleServerTesterWithDeps(key crypto.CipherKey, testdir string, gDeps, cDeps, tDeps, wDeps, hDeps, rDeps, hdbDeps, hcDeps, csDeps, apiDeps modules.Dependencies) (*serverTester, error) {
	...
	h, err := host.NewCustomHost(hDeps, cs, g, tp, w, mux, "localhost:0", filepath.Join(testdir, modules.HostDir), ":0", 5*time.Second)
	if err != nil {
		return nil, err
	}
	...
	srv, err := NewCustomServer(testdir, "localhost:0", "ScPrime-Agent", "", cs, nil, nil, g, h, m, r, tp, w, nil, nil, nil, apiDeps)
	if err != nil {
		return nil, err
	}
...
func assembleAuthenticatedServerTester(requiredPassword string, key crypto.CipherKey, testdir string, withPool bool) (*serverTester, error) {
	...
	h, err := host.New(cs, g, tp, w, mux, "localhost:0", filepath.Join(testdir, modules.HostDir), ":0", 5*time.Second)
	if err != nil {
		return nil, err
	}
