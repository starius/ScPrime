...
func NewAsync(APIaddr string, requiredUserAgent string, requiredPassword string, nodeParams node.NodeParams, loadStartTime time.Time) (*Server, <-chan error) {
	...
	s, err := func() (*Server, error) {
		...
		api := api.New(cfg, requiredUserAgent, requiredPassword, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
		srv := &Server{
			api: api,
			apiServer: &http.Server{
				Handler: api,
		...
		srv.node = n
		api.SetModules(n.ConsensusSet, n.Explorer, n.Gateway, n.Host, n.Miner, n.Renter, n.TransactionPool, n.Wallet, n.Downloader, n.Gui, n.MiningPool, n.StratumMiner)
		return srv, nil
	}()
	if err != nil {
		if n != nil {
