...
func newTestingGateway(t *testing.T) *Gateway {
	...
	g, err := New("127.0.0.1:0", false, build.TempDir("gateway", t.Name()))
	if err != nil {
		panic(err)
	}
	return g
...
func newNamedTestingGateway(t *testing.T, suffix string) *Gateway {
	...
	g, err := New("127.0.0.1:0", false, build.TempDir("gateway", t.Name()+suffix))
	if err != nil {
		panic(err)
	}
	return g
...
func TestNew(t *testing.T) {
	...
	if _, err := New("127.0.0.1:0", false, ""); err == nil {
		t.Fatal("expecting persistDir error, got nil")
	}
	...
	if _, err := New("127.0.0.1:0", false, dir); err == nil {
...
func TestManualConnectDisconnectPersist(t *testing.T) {
	...
	g2.Close()
	g2, err := New("127.0.0.1:0", false, g2.persistDir)
	if err != nil {
		t.Fatal(err)
	}
	defer g2.Close()
