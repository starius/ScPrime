...
func TestInitialBlockchainDownloadDisconnects(t *testing.T) {
	...
	g, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, "local", modules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	defer g.Close()
	...
	for i, rpcErr := range rpcErrs {
		g, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, "remote - "+strconv.Itoa(i), modules.GatewayDir))
		if err != nil {
			t.Fatal(err)
		}
		defer g.Close()
...
func TestInitialBlockchainDownloadDoneRules(t *testing.T) {
	...
	gatewayTimesout, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, "remote - timesout", modules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	// not-synced peers.
	gatewayNoTimeout, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, "remote - no timeout1", modules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	gatewayNoTimeout2, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, "remote - no timeout2", modules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < len(gatewayNoTimeouts); i++ {
		tmpG, err := gateway.New("127.0.0.1:0", false, filepath.Join(testdir, fmt.Sprintf("remote - no timeout-auto-%v", i+3), modules.GatewayDir))
		if err != nil {
			t.Fatal(err)
		}
		defer tmpG.Close()
