...
func TestSendShareNodesRequests(t *testing.T) {
	...
	mainGateway, err := gateway.New("127.0.0.1:0", true, build.TempDir("SiaNodeScannerTestGateway"))
	if err != nil {
	...
	for i := 0; i < numTestingGateways; i++ {
		g, err := gateway.New("127.0.0.1:0", true, build.TempDir(fmt.Sprintf("SiaNodeScannerTestGateway-%d", i)))
		if err != nil {
			t.Fatal("Error making new gateway: ", err)
		}
