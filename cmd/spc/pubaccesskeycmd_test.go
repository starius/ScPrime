...
func testAddKeyTwice(t *testing.T, c client.Client) {
	...
	keyName := "createkey1"
	_, err := skykeyCreate(c, keyName, pubaccesskey.TypePublicID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = skykeyCreate(c, keyName, pubaccesskey.TypePublicID)
...
func testInvalidSkykeyType(t *testing.T, c client.Client) {
	_, err := skykeyCreate(c, "createkey2", pubaccesskey.TypeInvalid)
...
func testSkykeyGet(t *testing.T, c client.Client) {
	...
	newSkykey, err := skykeyCreate(c, keyName, pubaccesskey.TypePublicID)
	if err != nil {
		t.Fatal(err)
	}
...
func testSkykeyListKeysAdditionalKeys(t *testing.T, c client.Client) {
	...
	for i := 0; i < nExtraKeys; i++ {
		...
		nextSkStr, err := skykeyCreate(c, nextName, pubaccesskey.TypePublicID)
		if err != nil {
			t.Fatal(err)
		}
